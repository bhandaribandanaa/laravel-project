...
$(document).ready( function () {
			...
			if ( $form.length > 0 ) {
				$('form.newsletter input[type="submit"]').bind('click', function ( event ) {
					...
					if ( validateEmail() ) { subscribe_newsletter($form)}
...
function subscribe_newsletter($form)
{
  ...
  var data = '';
  $.post(url, {email:$("#EMAIL").val(), _token : $("input[name='_token']").val()},function(data) {
      if(data.status=="failure") {
        $("#result1").hide().html("<div class='alert-danger'>"+data.message+"</div>").slideDown(200); 
      }
      else{
        $("#result1").hide().html("<div class='alert-success'>"+data.message+"</div>").slideDown(200); 
      }
  });
}
